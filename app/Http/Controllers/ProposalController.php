...
class ProposalController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->relation == 'Lead'){
            ...
            if($lead->leadStatus->name == 'Won'){
                $notification = array(
                    'message' => __("You can't add proposal to this lead, its already converted into customer!"),
                    'alert-type' => 'error'
                );
        ...
        if($validator){
            ...
            $notification = array(
                'message' => __('Proposal submitted successfully!'),
                'alert-type' => 'success'
            );
    ...
    public function mailToClient(Proposal $proposal){
        ...
        if($proposal->relation == 'Lead'){
            ...
            $to = $proposal->customer->first_contact->email;
        }
        ...
        if(CRMHelper::cron_status() == '1'){
            ...
            Mail::to($to)
            ->send(new ProposalMail($proposal, $company_details));
            $notification = array(
    ...
    public function update(Request $request, Proposal $proposal)
    {
        ...
        if($validator){
            ...
            $notification = array(
                'message' => __('Proposal updated successfully!'),
                'alert-type' => 'success'
            );
