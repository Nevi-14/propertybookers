...
class RoleController extends Controller
{
    ...
    public function default(Request $request, Role $role)
    {
        if($role->id == '1'){
            $notification = array(
                'message' => __("Admin can't be set as default role"),
                'alert-type' => 'error'
            );
            return back()->with($notification);            
        }
        ...
        if($validator){
            ...
            $notification = array(
                'message' => __('Role updated as default'),
                'alert-type' => 'success'
            );
            return back()->with($notification);
        }else{
    ...
    public function update(Request $request, Role $role)
    {
        ...
        if($role->id == '3'){
            $notification = [
                'alert-type'=>'error',
                'message'=>__("You can't update salesperson role")
            ];
            return back()->with($notification);            
        }
    ...
    public function destroy(Role $role)
    {
        if($role->id == '3'){
            $notification = [
                'alert-type'=>'error',
                'message'=>__("You can't delete salesperson role!")
            ];
            return back()->with($notification);            
        }
        ...
        if($role->delete()){
            $notification = [
                'alert-type'=>'success',
                'message'=>__('Role deleted successfully! Affected number of users are')." <b>".$affected_users.'</b> !'
            ];
            return back()->with($notification);
        }else{
            $notification = [
