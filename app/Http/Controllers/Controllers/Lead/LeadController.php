...
class LeadController extends Controller
{
    ...
    public function ajaxRequestPost(Request $request)
    {
        ...
        $lead = Lead::create([
           ...
           'last_name'=> $request->last_name,
            ...
            'total_properties'=> $request->total_properties,
            'lead_status_id'=>'6'
        ]);
  ...
  //Mail::to('sales@nomadux.com')
//->cc(['david.black@nomadux.com','samantha@nomadux.com','workemailnelson@gmail.com','info@propertybookers.us'])
//->send(new nomaduxContactForm($request));
      ...
      return response()->json([ 'success' =>  'Thank you '. "<span class='calendar-message  name mx-1'> $lead->first_name </span>". ' ' . "<span class='calendar-message name mx-1'>$request->last_name </span>". ' '. 'for your interest. We hope that you enjoyed reading our report'. "<span class='calendar-message active mx-1'>'The 5 biggest mistakes that Real Estate Investors make'</span>, now would you like to schedule an appointment for your free consultation with the founders of Nomadux?" ,'id'=>$lead->id]);
    }



    public function ApiLeadUpdate(Request $request )
    {
 ...
 $Note = Note::where(['lead_id'=>$request->id])->update(['note' => 'Appointment  scheduled on '.Carbon::createFromFormat('Y-m-d', $request->appoiment)->format('m/d/Y')  .' at ' . $request->time . ' ' .  $request->timezone . '  time you can check your appoiment in the Reminders options.']);

    $reminder = Reminder::create([
        'description'=>'Appointment  scheduled on '.Carbon::createFromFormat('Y-m-d', $request->appoiment)->format('m/d/Y')  .' at ' . $request->time . ' ' .  $request->timezone . '  time.',
        'user_id'=>1,  // Remainder owner
        'date'=>$request->appoiment,
 ...
 return response()->json([ 'success' => 'Thank you for scheduling an appointment , we will contact you on '. Carbon::createFromFormat('Y-m-d', $request->appoiment)->format('m/d/Y')  .' at ' . $request->time . ' ' .  $request->timezone . ' time.']);
    ...
    public function webToLeadForm(Request $request){
        // , $token
...
Mail::send(new ContactMail($request));
      ...
      //  $url = $form->returnurl;
      //,  compact('url')
       return view('layouts.webform_success');  
    ...
    public function update(Request $request, Lead $lead)
    {
        ...
        $lead->total_properties = $request->total_properties;
