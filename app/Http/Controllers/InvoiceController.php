...
class InvoiceController extends Controller
{
    ...
    public function mailToClient(Invoice $invoice){
        ...
        $to = $invoice->customer->first_contact->email;
    ...
    public function pay_invoice(Request $request)
    {
        ...
        if($request->has('txn_receipt')){
           ...
           $invoice->txn_receipt = $name;
            ...
            if($today->diff($due_date)->invert == 1){
                $notification = array(
                    'message' => __('Due date has been passed for this invoice, Adjust the due date or create new invoice!'),
                    'alert-type' => 'error'
                );
                return back()->with($notification);   
            }
        ...
        if($invoice->save()){
            $notification = array(
                'message' => __('Your invoice paid successfully, please wait for the confirmation!'),
                'alert-type' => 'success'
            );
            return back()->with($notification);
        }else{
    ...
    {
        $validator = $request->validate([
            'invoice_number'=>'required | unique:invoices,invoice_number,'.$invoice->invoice_number.',invoice_number',
            'invoice_title'=>'required',
        ]);
        if($validator){
