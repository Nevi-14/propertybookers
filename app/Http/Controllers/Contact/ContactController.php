...
class ContactController extends Controller
{
    ...
    public function importStore(Request $request){
        ...
        if(count($import->errors()) == 0){
            $notification = array(
                'message' => $import->getRowCount().' '.__('customers imported successfully!'),
                'alert-type' => 'success'
            );
    ...
    public function leadToCustomer(Lead $lead, Request $request){
        ...
        $notification = array(
            'message' => __('Lead converted into customer successfully!'),
            'alert-type' => 'success'
        );
    ...
    public function mailToClient(Customer $customer, $password=false){
        ...
        $notification = array(
            'message' => __('Contact added and notified successfully!'),
            'alert-type' => 'success'
        );
        return redirect(url('customer'))->with($notification);
    }
    ...
    public function makeContactPrimary(Request $request, Contact $contact){
        {
            ...
            if($old_primary_contact->id == $contact->id){
                $notification = array(
                    'message' => __('You have to keep atleat one contact as primary!'),
                    'alert-type' => 'error'
                );
                return back()->with($notification);                
            }
    ...
    public function destroy(Customer $customer)
    {
        ...
        if($customer->delete()){
            $notification = [
                'alert-type'=>'success',
                'message'=>__('Customer and his contacts deleted successfully!')
            ];
            return back()->with($notification);
        }
    }
    ...
    public function destroyContact(Contact $contact)
    {
        if($contact->is_primary == 'yes'){
            $notification = [
                'alert-type'=>'error',
                'message'=>__('You can not delete primary contact, it is bound with the customer ID!')
            ];
            return back()->with($notification);            
        }
