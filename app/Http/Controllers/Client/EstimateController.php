...
class EstimateController extends Controller
{
    public function index()
    {
        ...
        $user = Auth::user();
        $estimates = Estimate::where('customer_id',session('client_id'))->orderby('id','desc')->get();
        ...
        return view('client.estimate.index', compact(['route_active', 'estimates','estimateProducts','base_currency','salesperson','currencies','company_details']));  
    }
    ...
    public function mailToClient(Estimate $estimate){
        ...
        $notification = array(
            'message' => __('Mail added in the queue successfully!'),
            'alert-type' => 'success'
        );
    ...
    public function store(Request $request)
    {
        ...
        if($validator){
            ...
            $notification = array(
                'message' => __('Estimate submitted successfully!'),
                'alert-type' => 'success'
            );
            return back()->with($notification);
        }else{
    ...
    public function update(Request $request, estimate $estimate)
    {
        ...
        if($validator){
            ...
            $estimate->mail_to = $request->mail_to;
            ...
            $notification = array(
                'message' => __('Estimate updated successfully!'),
                'alert-type' => 'success'
            );
            return back()->with($notification);
        }else{
    ...
    public function convertToInvoice(Estimate $estimate, $token){
        if ($estimate->invoiced_token != $token) {
            ...
            $invoice = Invoice::create([
                ...
                // estimate_paid',['yes', 'no'])->default('no');// this estimate paid status yes/no
            ...
            // return (new InvoiceMail($invoice, $company_details))->render();
            ...
            $notification = [
                'message'=>__('Your invoice has been created and dispatched successfully, you will receive an email shortly!'),
                'alert-type' => 'success'
            ];
            return back()->with($notification);
        }
