...
class TaskController extends Controller
{
    ...
    {
        $route_active = 'task';
        $task_owners = User::where('role_id','!=','1')->where('status','active')->get();
        $user = Auth::user();
        if ($user->role_id != '1') {
    ...
    public function create()
    {
        ...
        $task_owner = User::where('role_id','!=','1')->where('status','active')->get();
        return view('crm.task.create', compact(['route_active','task_owner']));  
    }
    ...
    public function store(Request $request)
    {
        if($request->relation == 'Lead'){
            ...
            if($lead->leadStatus->name == 'Won'){
                $notification = array(
                    'message' => __("You can't add task to this lead! This is already a customer."),
                    'alert-type' => 'error'
                );
        ...
        if($validator){
            ...
            $notification = array(
                'message' => __('Task submitted successfully!'),
                'alert-type' => 'success'
            );
    ...
    public function mailToStaff(Task $task){
        ...
        if($task->relation == 'Lead'){
            $to = $task->lead->email;
        }else{
            $to = $task->customer->first_contact->email;
        }

        if(CRMHelper::cron_status() == '1'){
            StaffEmailJob::dispatch($task, $currency);
            $notification = array(
                'message' => __('Mail added in the queue successfully and will be sent shortly!'),
                'alert-type' => 'success'
            );
        }else{
            Mail::to($to)
            ->send(new TaskMail($task, $currency));
            $notification = array(
