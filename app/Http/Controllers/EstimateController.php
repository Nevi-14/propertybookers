...
class EstimateController extends Controller
{
    ...
    public function mailToClient(Estimate $estimate){
        ...
        if($estimate->relation == 'Lead'){
            $to = $estimate->lead->email;
        }else{
            $to = $estimate->customer->first_contact->email;
        }
        ...
        if(CRMHelper::cron_status() == '1'){
            ...
            Mail::to($to)
            ->send(new EstimateMail($estimate, $company_details));
            $notification = array(
    ...
    public function convertToInvoice(Estimate $estimate, $token){
        if ($estimate->invoiced_token != $token) {
            ...
            if(CRMHelper::cron_status() == '1'){
                ...
                Mail::to($to)
                ->send(new InvoiceMail($invoice, $company_details));
            }
