<script>
  ...
  var invoice_relation = "Customer";

  if(invoice_relation == 'Customer'){
    getCustomers();
  }

  function getCustomers(){
    var customer_id = "{{$invoice->customer_id}}";
  ...
  function invoiceProducts(){
    ...
    var url="{{url('api/invoice/getInvoiceProducts')}}";
    $.ajaxSetup({
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
    ...
    $.ajax({
      ...
      dataType: 'JSON',
      success: function (data) { 
        row_id = data.invoice_products.length+1;
  ...
  // Add product in the invoice from the products list
